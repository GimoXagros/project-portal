...
import { downloadItems, releaseIdentity } from './release-utils.mjs'

export async function fetchRelease(url, token, fetcher = fetch) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    let response
    try {
      response = await fetcher(url, { headers, signal: AbortSignal.timeout(15000) })
    } catch {
      if (attempt < 2) { await new Promise((done) => setTimeout(done, 500 * (attempt + 1))); continue }
      throw new Error('NETWORK: 일시적인 네트워크 오류 또는 15초 요청 시간 초과 (3회 시도)')
    }
    ...
    if (response.status >= 500 && attempt < 2) { await new Promise((done) => setTimeout(done, 500 * (attempt + 1))); continue }
    if (!response.ok) throw new Error(`${response.status >= 500 ? 'NETWORK' : 'API_ACCESS'}: GitHub API HTTP ${response.status}`)
    try { return await response.json() } catch { throw new Error('NETWORK: GitHub API 응답이 유효한 JSON이 아닙니다') }
  }
}

export async function verifyProject(project, { token, fetcher = fetch } = {}) {
  ...
  let release
  try { release = await fetchRelease(`https://api.github.com/repos/${owner}/${repo}/releases/tags/${encodeURIComponent(tag)}`, token, fetcher) }
  ...
  compare('version/tag_name', project.version, release.tag_name)
...
async function main() {
  ...
  for (const project of projects) {
    ...
    if (!errors.length) console.log(`[${project.id}] ${project.version}: 공개 Release 날짜·URL·자산 검증 성공`)
  }
