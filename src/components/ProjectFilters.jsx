...
export default function ProjectFilters({ query, setQuery, type, setType, platform, setPlatform, platforms, resultCount }) {
  ...
  return <div className="filters" aria-label="프로젝트 필터">
    ...
    <div className="filter-bottom">
      ...
      <div><span className="result-count" role="status" aria-live="polite" aria-atomic="true">{resultCount ? `${resultCount}개 표시` : '검색 결과 0개'}</span>{active && <button className="reset-button" type="button" onClick={reset}><X size={15} /> 필터 초기화</button>}</div>
    </div>
  </div>
}
