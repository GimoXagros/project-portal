  ...
  return <main className="project-detail" id="main-content">
    <div className="detail-hero">
      ...
      <div className="detail-hero-grid">
        <div><div className="card-topline"><span className="type-label">{typeLabels[project.type] || typeLabels.other}</span>{project.demo && <span className="demo-badge">DEMO · SAMPLE DATA</span>}</div><h1>{project.title}</h1>{project.titleOriginal && <p className="detail-original">{project.titleOriginal}</p>}<p className="detail-subtitle">{project.subtitle}</p></div>
        <div className={`detail-cover tone-${project.coverTone || 'slate'}`}><span>{project.id.slice(-2)}</span><small>{project.demo ? 'LAYOUT PREVIEW ONLY' : project.id}</small></div>
      </div>
    ...
    <div className="detail-layout section-pad">
      <div className="detail-main">
        <section className="detail-section intro-section"><div className="detail-section-title"><span>OVERVIEW</span><h2>프로젝트 소개</h2></div><p className="lead-description">{project.description}</p>{project.demo && <div className="demo-panel"><strong>DEMO CONTENT</strong><p>이 항목은 기능과 레이아웃 확인용입니다. 실제 게임, 패치, Releases 자산 또는 저장소를 나타내지 않습니다.</p></div>}</section>
      ...
      <aside className="detail-aside" aria-label="프로젝트 부가 정보">
        ...
        {project.credits?.length > 0 && <div className="aside-card"><h2>Credits</h2><ul>{project.credits.map((credit, index) => <li key={`${credit}-${index}`}>{credit}</li>)}</ul></div>}
